import type { ConfigOption as ApiConfigOption } from 'ardunno-cli';
import clone from 'clone';
import deepEqual from 'deep-equal';

export type { ApiConfigOption as ConfigOption };
...
export class FQBN {
   ...
   */
  withConfigOptions(...configOptions: ApiConfigOption[]): FQBN {
    ...
    const options: Mutable<ConfigOption> = this.options
      ? clone(this.options)
      : {};
    let didUpdate = false;
...
function serialize(
  ...
  arch: string,
  boardId: string,
  options: ConfigOption | undefined = undefined
): string {
...
type ConfigOption = Record<string, string>;
